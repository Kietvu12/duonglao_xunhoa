...
import { baiVietSuKienAPI, suKienAPI, uploadAPI } from '../../services/api';
import RichTextEditor from '../../components/RichTextEditor';
import { normalizeImageUrl } from '../../utils/imageUtils';

export default function BaiVietSuKienPage() {
  ...
  // States cho searchable dropdown sự kiện
  const [suKienSearch, setSuKienSearch] = useState('');
  const [showSuKienDropdown, setShowSuKienDropdown] = useState(false);
  const [filteredSuKiens, setFilteredSuKiens] = useState([]);
  ...
  const loadSuKiens = async () => {
    try {
      // Truyền limit=-1 để lấy tất cả sự kiện
      const response = await suKienAPI.getAll({ limit: -1 });
      setSuKiens(response.data || []);
      setFilteredSuKiens(response.data || []);
    } catch (error) {
      console.error('Error loading su kiens:', error);
    }
  };

  // Lọc sự kiện khi người dùng gõ
  useEffect(() => {
    if (suKienSearch.trim() === '') {
      setFilteredSuKiens(suKiens);
    } else {
      const filtered = suKiens.filter(dv => 
        dv.tieu_de.toLowerCase().includes(suKienSearch.toLowerCase())
      );
      setFilteredSuKiens(filtered);
    }
  }, [suKienSearch, suKiens]);

  // Đóng dropdown khi click bên ngoài
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (showSuKienDropdown && !event.target.closest('.sukien-search-container')) {
        setShowSuKienDropdown(false);
      }
    };
  ...
  const loadBaiViets = async () => {
    try {
      setLoading(true);
      // Truyền limit=-1 để lấy tất cả bài viết sự kiện
      const response = await baiVietSuKienAPI.getAll({ limit: -1 });
      setBaiViets(response.data || []);
    } catch (error) {
      console.error('Error loading bai viets:', error);
      alert('Lỗi khi tải danh sách bài viết sự kiện: ' + error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing) {
        await baiVietSuKienAPI.update(editing.id, submitData);
        alert('Cập nhật bài viết sự kiện thành công');
      } else {
        await baiVietSuKienAPI.create(submitData);
        alert('Tạo bài viết sự kiện thành công');
      }
      setShowModal(false);
      setEditing(null);
      resetForm();
  ...
  const handleEdit = async (bv) => {
    try {
      const fullArticle = await baiVietSuKienAPI.getById(bv.id);
      const articleData = fullArticle.data;
      
      setEditing(articleData);
      setFormData({
        id_su_kien: articleData.id_su_kien || '',
      ...
      // Set tên sự kiện cho search field
      if (articleData.id_su_kien) {
        const selectedSuKien = suKiens.find(dv => dv.id === articleData.id_su_kien);
        if (selectedSuKien) {
          setSuKienSearch(selectedSuKien.tieu_de);
        }
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await baiVietSuKienAPI.delete(id);
      alert('Xóa bài viết sự kiện thành công');
      loadBaiViets();
    } catch (error) {
      alert('Lỗi: ' + error.message);
    }
  ...
  const handlePublish = async () => {
    ...
    try {
      if (editing) {
        await baiVietSuKienAPI.update(editing.id, submitData);
        alert('Xuất bản bài viết sự kiện thành công');
      } else {
        await baiVietSuKienAPI.create(submitData);
        alert('Xuất bản bài viết sự kiện thành công');
      }
      ...
      {!showModal && (
        <div className="space-y-6">
          ...
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-4xl font-black leading-tight tracking-tight text-gray-800">Bài viết Sự kiện</h1>
              <p className="text-gray-600 mt-2">Quản lý bài viết giới thiệu sự kiện</p>
            </div>
            <button
              onClick={() => {
                resetForm();
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-12 text-center">
              ...
              <p className="text-gray-500 text-lg mb-2">Chưa có bài viết sự kiện nào</p>
            ...
            <div className="max-w-7xl mx-auto">
              <div className="mb-8">
                <h1 className="text-4xl font-black leading-tight tracking-tight">
                  {editing ? 'Chỉnh sửa Bài viết Sự kiện' : 'Tạo Bài viết Sự kiện Mới'}
                </h1>
              </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <div className="lg:col-span-2 flex flex-col gap-6">
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 p-6">
                      <div className="flex flex-col w-full relative sukien-search-container">
                        <p className="text-base font-medium leading-normal pb-2">Sự kiện liên quan</p>
                        <div className="relative">
                          <input
                            type="text"
                            value={suKienSearch}
                            onChange={(e) => {
                              setSuKienSearch(e.target.value);
                              setShowSuKienDropdown(true);
                              // Reset id_su_kien khi người dùng thay đổi text
                              if (formData.id_su_kien) {
                                const selectedSuKien = suKiens.find(dv => dv.id === formData.id_su_kien);
                                if (selectedSuKien && selectedSuKien.tieu_de !== e.target.value) {
                                  setFormData({ ...formData, id_su_kien: '' });
                                }
                              }
                            }}
                            onFocus={() => setShowSuKienDropdown(true)}
                            placeholder="Gõ để tìm kiếm sự kiện..."
                        ...
                        {/* Dropdown list */}
                        {showSuKienDropdown && filteredSuKiens.length > 0 && (
                          <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-y-auto z-50">
                            {filteredSuKiens.map((dv) => (
                              <div
                                key={dv.id}
                                onClick={() => {
                                  setFormData({ ...formData, id_su_kien: dv.id });
                                  setSuKienSearch(dv.tieu_de);
                                  setShowSuKienDropdown(false);
                                }}
                                className={`px-4 py-3 cursor-pointer hover:bg-blue-50 transition-colors ${
                                  formData.id_su_kien === dv.id ? 'bg-blue-100 font-medium' : ''
                                }`}
                              >
                                {dv.tieu_de}
                              </div>
                        ...
                        {showSuKienDropdown && suKienSearch && filteredSuKiens.length === 0 && (
                          <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg p-4 z-50">
                            <p className="text-gray-500 text-center">Không tìm thấy sự kiện</p>
                          </div>
                        )}
                      </div>
                    </div>
