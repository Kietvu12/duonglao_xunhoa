...
import { baiVietPhongAPI, loaiPhongAPI, uploadAPI } from '../../services/api';
import RichTextEditor from '../../components/RichTextEditor';
import { normalizeImageUrl } from '../../utils/imageUtils';

export default function BaiVietPhongPage() {
  ...
  const loadBaiViets = async () => {
    try {
      setLoading(true);
      const response = await baiVietPhongAPI.getAll();
      setBaiViets(response.data || []);
    } catch (error) {
      console.error('Error loading bai viets:', error);
      alert('Lỗi khi tải danh sách bài viết phòng: ' + error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing) {
        await baiVietPhongAPI.update(editing.id, submitData);
        alert('Cập nhật bài viết phòng thành công');
      } else {
        await baiVietPhongAPI.create(submitData);
        alert('Tạo bài viết phòng thành công');
      }
      setShowModal(false);
      setEditing(null);
      resetForm();
  ...
  const handleEdit = async (bv) => {
    try {
      const fullArticle = await baiVietPhongAPI.getById(bv.id);
      const articleData = fullArticle.data;
      
      setEditing(articleData);
      setFormData({
        id_loai_phong: articleData.id_loai_phong || '',
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await baiVietPhongAPI.delete(id);
      alert('Xóa bài viết phòng thành công');
      loadBaiViets();
    } catch (error) {
      alert('Lỗi: ' + error.message);
    }
  ...
  const handlePublish = async () => {
    ...
    try {
      if (editing) {
        await baiVietPhongAPI.update(editing.id, submitData);
        alert('Xuất bản bài viết phòng thành công');
      } else {
        await baiVietPhongAPI.create(submitData);
        alert('Xuất bản bài viết phòng thành công');
      }
      ...
      {!showModal && (
        <div className="space-y-6">
          ...
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-4xl font-black leading-tight tracking-tight text-gray-800">Bài viết Phòng</h1>
              <p className="text-gray-600 mt-2">Quản lý bài viết giới thiệu phòng</p>
            </div>
            <button
              onClick={() => {
                resetForm();
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-12 text-center">
              ...
              <p className="text-gray-500 text-lg mb-2">Chưa có bài viết phòng nào</p>
            ...
            <div className="max-w-7xl mx-auto">
              <div className="mb-8">
                <h1 className="text-4xl font-black leading-tight tracking-tight">
                  {editing ? 'Chỉnh sửa Bài viết Phòng' : 'Tạo Bài viết Phòng Mới'}
                </h1>
              </div>
                  ...
                  <div className="lg:col-span-2 flex flex-col gap-6">
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 p-6">
                      <label className="flex flex-col w-full">
                        <p className="text-base font-medium leading-normal pb-2">Loại phòng liên quan</p>
                        <select
                          value={formData.id_loai_phong}
                          onChange={(e) => setFormData({ ...formData, id_loai_phong: e.target.value })}
                        ...
                        >
                          ...
                          {loaiPhongs.map((lp) => (
                            <option key={lp.id} value={lp.id}>Loại phòng #{lp.id}</option>
                          ))}
                        </select>
                      </label>
                    </div>
