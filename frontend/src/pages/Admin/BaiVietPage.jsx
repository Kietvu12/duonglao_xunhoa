...
export default function BaiVietPage() {
  ...
  const handlePublish = async () => {
    ...
    // Kiểm tra nếu có ngày đăng trong tương lai thì giữ trạng thái "nhap"
    // Chỉ set "xuat_ban" khi không có ngay_dang hoặc ngay_dang <= hiện tại
    let finalTrangThai = 'xuat_ban';
    if (formData.ngay_dang) {
      const ngayDang = new Date(formData.ngay_dang);
      const now = new Date();
      if (ngayDang > now) {
        // Nếu ngày đăng trong tương lai, giữ trạng thái "nhap"
        finalTrangThai = 'nhap';
      }
    }
    ...
    try {
      let successMessage = 'Xuất bản bài viết thành công';
      if (finalTrangThai === 'nhap' && formData.ngay_dang) {
        const ngayDang = new Date(formData.ngay_dang);
        successMessage = `Đã lên lịch xuất bản bài viết vào ${ngayDang.toLocaleString('vi-VN')}`;
      }
  ...
  return (
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
              <h1 className="text-4xl font-black leading-tight tracking-tight text-gray-800">Quản lý Bài viết</h1>
              <p className="text-gray-600 mt-2">Danh sách bài viết và tin tức</p>
        </div>
        <button
          onClick={() => {
            resetForm();
          ...
          <header className="flex items-center justify-between border-b border-gray-200 px-6 py-3 bg-white sticky top-0 z-10">
            ...
            <div className="flex items-center gap-2">
              {/* <button
                type="button"
                onClick={() => {
                  // Preview functionality - có thể mở trong tab mới
                  window.open(`/bai-viet/preview/${editing?.id || 'new'}`, '_blank');
                }}
                className="flex min-w-[84px] max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-10 px-4 bg-[#4A90E2]/20 text-[#4A90E2] text-sm font-bold leading-normal tracking-[0.015em] hover:bg-[#4A90E2]/30"
              >
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 p-6">
                      <label className="flex flex-col w-full">
                        ...
                        <textarea
                          ...
                          placeholder="Viết mô tả ngắn về bài viết (sẽ hiển thị trong danh sách bài viết)..."
                          maxLength={500}
                        />
                        ...
                        {formData.anh_dai_dien ? (
                          <div className="relative">
                            <img
                              ...
                              onError={(e) => {
                                console.error('Error loading thumbnail image:', formData.anh_dai_dien);
                                e.target.style.display = 'none';
                              }}
                              onLoad={() => {
                                console.log('Thumbnail image loaded successfully:', formData.anh_dai_dien);
                              }}
                            />
                            <button
                              type="button"
                      ...
                      <div className="flex flex-col gap-4">
                        <label className="flex flex-col w-full">
                          ...
                          >
                            ...
                            <option value="Medical News">Tin tức y tế</option>
                          </select>
                        </label>
                        <label className="flex flex-col w-full">
